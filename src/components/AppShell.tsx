...
type IconName = 'overview' | 'id' | 'members' | 'community' | 'saved' | 'notifications' | 'career' | 'wealth' | 'wallet' | 'membership' | 'support' | 'admin';
...
function SeaIcon({ name }: { name: IconName }) {
  ...
  const shapes: Record<IconName, React.ReactNode> = {
    ...
    notifications: <><path {...common} d="M6 9a6 6 0 0 1 12 0c0 7 3 7 3 7H3s3 0 3-7" /><path {...common} d="M10 20h4" /></>,
...
function UtilityIcon({ type }: { type: 'search' | 'community' | 'wallet' | 'notifications' }) {
  ...
  if (type === 'notifications') return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 9a6 6 0 0 1 12 0c0 7 3 7 3 7H3s3 0 3-7M10 20h4" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round"/></svg>;
...
export default function AppShell({ title, kicker, children }: { title: string; kicker: string; children: React.ReactNode }) {
  ...
  useEffect(() => {
    void fetch('/api/notifications?limit=1').then(async response => {
  ...
  return <main className="appPage">
    <aside className="sidebar">
      ...
      <nav aria-label="Member navigation">{links.map(({ label, href, icon }) => { const active = isActive(href); return <Link key={href} href={href} className={active ? 'active' : undefined} aria-current={active ? 'page' : undefined}><SeaIcon name={icon} /><span>{label}</span>{href === '/notifications' && unread > 0 && <b className="sideUnread">{unread > 99 ? '99+' : unread}</b>}</Link>; })}</nav>
    ...
    <section className="appContent">
      <div className="appUtilityBar" aria-label="Quick actions">
        ...
        <Link className="appUtilityIcon notificationUtility" href="/notifications" aria-label="Open notifications"><UtilityIcon type="notifications" />{unread > 0 && <span className="notificationBadge">{unread > 99 ? '99+' : unread}</span>}</Link>
      ...
      <div className="appHeader"><div><span className="microLabel">{kicker}</span><h1>{title}</h1></div><div className="statusPill"><span className="liveDot" /> Member workspace</div></div>
      {children}
    </section>

    <nav className="mobileDock" aria-label="Mobile member navigation">{mobileLinks.map(({ label, href, icon }) => { const active = isActive(href); return <Link key={href} href={href} className={active ? 'active' : undefined} aria-current={active ? 'page' : undefined}><SeaIcon name={icon} /><span>{label}</span></Link>; })}</nav>
  </main>;
}
