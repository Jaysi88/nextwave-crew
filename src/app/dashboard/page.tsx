    ...
    <div className="dashboardGrid">
      <section className="panel span2">
        <div className="panelHead">
          <div><span className="microLabel">CURRENT VOYAGE</span><h2>{member.role}</h2><p>{member.ship} · {member.country}</p></div>
        ...
        <div className="panelFooter"><span>{member.daysAtSea} days completed</span><span>74 days estimated remaining</span></div>
      </section>
      ...
      <section className="panel span3"><div className="panelHead"><div><span className="microLabel">THIS CONTRACT</span><h2>Financial flight plan</h2></div><span className="statusPill positive"><span className="liveDot" /> On track</span></div><div className="moneyGrid"><div><span>Contract income</span><b>${wealth.contractIncome.toLocaleString()}</b><small>Projected</small></div><div><span>Shore reserve</span><b>${wealth.shoreReserve.toLocaleString()}</b><small>Protected runway</small></div><div><span>Emergency fund</span><b>${wealth.emergencyFund.toLocaleString()}</b><small>Safety buffer</small></div><div><span>Invested</span><b>${wealth.invested.toLocaleString()}</b><small>Tracked assets</small></div></div></section>
    </div>
  </AppShell>;
}
