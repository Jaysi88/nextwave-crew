...
export default function CommunityPage() {
  ...
  async function loadCommunities() {
    ...
    if (r.ok) { setCommunities(j.communities); setActive((current: Community | null) => current ? j.communities.find((c: Community) => c.id === current.id) ?? current : j.communities[0] ?? null); }
  ...
  async function loadPosts(community?: Community | null) {
    ...
    const r = await fetch(`/api/community/posts?communityId=${encodeURIComponent(community.id)}`); const j = await r.json();
    if (r.ok) setPosts(j.posts);
  }
  useEffect(() => { loadCommunities(); }, []);
  useEffect(() => { loadPosts(active); }, [active?.id, active?.joined]);
  async function toggleJoin(c: Community) {
    ...
    const j = await r.json(); if (!r.ok) setMessage(j.error || 'Unable to update membership.'); else { setMessage(c.joined ? 'Community left.' : 'Welcome to the community.'); await loadCommunities(); }
  }
  async function post(event: FormEvent<HTMLFormElement>) {
    ...
    const j = await r.json(); if (!r.ok) setMessage(j.error || 'Unable to post.'); else { event.currentTarget.reset(); setMessage('Post published.'); await loadPosts(active); }
  }
  return <AppShell title="Crew Community" kicker="PROFESSIONAL NETWORK">
    ...
    <div className="communityGrid">{communities.map((c, i) => <article className={`communityCard ${active?.id === c.id ? 'featuredPrice' : ''}`} key={c.id}>
      ...
      <div><button className="ghostButton small" onClick={() => setActive(c)}>Open space</button> <button className="ghostButton small" onClick={() => toggleJoin(c)}>{c.joined ? 'Leave' : 'Join'}</button></div>
    </article>)}</div>
