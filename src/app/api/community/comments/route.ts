...
export async function GET(request: Request) {
  ...
  const rows = await member.sql`select co.id, co.author_id, co.body, co.created_at, p.display_name, p.avatar_url, p.verification
...
export async function POST(request: Request) {
  ...
  if (!postId || body.length < 1 || body.length > 5000) {
    return Response.json({ error: 'Valid postId and comment body are required.' }, { status: 400 });
  }
  ...
  const author = await member.sql`select author_id from posts where id = ${postId} limit 1`;
  const authorId = String(author[0]?.author_id || '');
  if (authorId && authorId !== member.user.id) {
    await member.sql`insert into notifications (user_id, type, title, body, action_url)
      select ${authorId}, 'post_comment', 'New comment on your post', p.display_name || ' joined the conversation.', '/community'
