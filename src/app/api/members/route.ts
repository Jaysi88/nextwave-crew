...
export async function GET(request: Request) {
  ...
  const pattern = `%${q}%`;
  const rows = await member.sql`select p.id, p.display_name, p.avatar_url, p.cover_url, p.nationality, p.country_of_residence, p.verification,
      cp.department, cp.current_position, cp.ship_segment, cp.years_at_sea, cp.career_summary, cp.open_to_work,
      cp.open_to_mentor, cp.looking_for_mentor,
    ...
    order by p.verification = 'verified' desc, cp.open_to_mentor desc, cp.open_to_work desc, p.display_name asc
    limit 100`;
